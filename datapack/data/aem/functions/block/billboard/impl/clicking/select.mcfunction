...
execute as @e[type=marker,tag=aem.billboard.center] if score @s aem.id = .temp aem.data at @s run summon armor_stand ~ ~-0.5 ~ {Invisible:1b,Marker:1b,Invulnerable:1b,NoGravity:1b,Tags:["aem.billboard","aem.billboard.display","aem.setup"],ArmorItems:[{},{},{},{id:"minecraft:quartz",Count:1b}]}
